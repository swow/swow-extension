...
PHP_FUNCTION(swow_curl_multi_init)
{
    ...
    object_init_ex(return_value, swow_curl_multi_ce);
    mh = Z_CURL_MULTI_P(return_value);
    mh->multi = multi;

    zend_llist_init(&mh->easyh, sizeof(zval), _swow_curl_multi_cleanup_list, 0);
}
/* }}} */

/* {{{ Add a normal cURL handle to a cURL multi handle */
PHP_FUNCTION(swow_curl_multi_add_handle)
{
    zval      *z_mh;
    zval      *z_ch;
    php_curlm *mh;
    ...
    ZEND_PARSE_PARAMETERS_START(2,2)
        ...
        Z_PARAM_OBJECT_OF_CLASS(z_ch, swow_curl_ce)
...
PHP_FUNCTION(swow_curl_multi_remove_handle)
{
    zval      *z_mh;
    zval      *z_ch;
    php_curlm *mh;
...
PHP_FUNCTION(swow_curl_multi_getcontent)
{
    zval     *z_ch;
    php_curl *ch;
...
PHP_FUNCTION(swow_curl_multi_info_read)
{
    zval      *z_mh;
    php_curlm *mh;
    CURLMsg      *tmp_msg;
...
PHP_FUNCTION(swow_curl_multi_errno)
{
    zval        *z_mh;
    php_curlm   *mh;
...
PHP_FUNCTION(swow_curl_multi_strerror)
{
    zend_long code;
    const char *str;
...
PHP_FUNCTION(swow_curl_multi_setopt)
{
    zval       *z_mh, *zvalue;
    zend_long        options;
    php_curlm *mh;
...
static zend_object_handlers swow_curl_multi_handlers;
...
static zend_object *curl_multi_create_object(zend_class_entry *class_type) {
    ...
    intern->std.handlers = &swow_curl_multi_handlers;
#endif
...
void swow_curl_multi_register_handlers(void) {
    swow_curl_multi_ce->create_object = curl_multi_create_object;
#if PHP_VERSION_ID >= 80300
    swow_curl_multi_ce->default_object_handlers = &swow_curl_multi_handlers;
#endif
    ...
    swow_curl_multi_handlers.clone_obj = NULL;
    swow_curl_multi_handlers.cast_object = swow_curl_cast_object;
    swow_curl_multi_handlers.compare = zend_objects_not_comparable;
}

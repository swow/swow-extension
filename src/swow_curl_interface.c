...
zend_class_entry *swow_curl_ce;
zend_class_entry *swow_curl_share_ce;
static zend_object_handlers swow_curl_object_handlers;
...
#if PHP_VERSION_ID < 80300
    intern->std.handlers = &swow_curl_object_handlers;
#endif
...
static zend_object *curl_clone_obj(zend_object *object) {
    ...
    clone_object = curl_create_object(swow_curl_ce);
    clone_ch = swow_curl_from_obj(clone_object);
    swow_init_curl_handle(clone_ch);

    ch = swow_curl_from_obj(object);
    ...
    clone_ch->cp = cp;
    _swow_setup_easy_copy_handlers(clone_ch, ch);
...
static HashTable *curl_get_gc(zend_object *object, zval **table, int *n)
{
    php_curl *curl = swow_curl_from_obj(object);
...
PHP_FUNCTION(swow_curl_version)
{
    curl_version_info_data *d;
...
php_curl *swow_swow_init_curl_handle_into_zval(zval *curl)
{
    ...
    object_init_ex(curl, swow_curl_ce);
    ch = Z_CURL_P(curl);

    swow_init_curl_handle(ch);
...
PHP_FUNCTION(swow_curl_init)
{
    ...
    ch = swow_swow_init_curl_handle_into_zval(return_value);
...
static inline zend_result build_mime_structure_from_hash(php_curl *ch, zval *zpostfields) /* {{{ */
{
    ...
    ZEND_HASH_FOREACH_KEY_VAL(postfields, num_key, string_key, current) {
        ...
        if (Z_TYPE_P(current) == IS_OBJECT &&
                instanceof_function(Z_OBJCE_P(current), swow_curl_CURLFile_class)) {
            ...
            prop = zend_read_property_ex(swow_curl_CURLFile_class, Z_OBJ_P(current), ZSTR_KNOWN(ZEND_STR_NAME), /* silent */ false, &rv);
            ZVAL_DEREF(prop);
            if (Z_TYPE_P(prop) != IS_STRING) {
                ...
                prop = zend_read_property(swow_curl_CURLFile_class, Z_OBJ_P(current), "mime", sizeof("mime")-1, 0, &rv);
                ZVAL_DEREF(prop);
                if (Z_TYPE_P(prop) == IS_STRING && Z_STRLEN_P(prop) > 0) {
                    type = Z_STRVAL_P(prop);
                }
                prop = zend_read_property(swow_curl_CURLFile_class, Z_OBJ_P(current), "postname", sizeof("postname")-1, 0, &rv);
                ZVAL_DEREF(prop);
                if (Z_TYPE_P(prop) == IS_STRING && Z_STRLEN_P(prop) > 0) {
        ...
        if (Z_TYPE_P(current) == IS_OBJECT && instanceof_function(Z_OBJCE_P(current), swow_curl_CURLStringFile_class)) {
            /* new-style file upload from string */
            zval *prop, rv;
            char *type = NULL, *filename = NULL;

            prop = zend_read_property(swow_curl_CURLStringFile_class, Z_OBJ_P(current), "postname", sizeof("postname")-1, 0, &rv);
            if (EG(exception)) {
                zend_string_release_ex(string_key, 0);
                return FAILURE;
            }
            ...
            prop = zend_read_property(swow_curl_CURLStringFile_class, Z_OBJ_P(current), "mime", sizeof("mime")-1, 0, &rv);
            if (EG(exception)) {
                zend_string_release_ex(string_key, 0);
                return FAILURE;
            }
            ...
            prop = zend_read_property(swow_curl_CURLStringFile_class, Z_OBJ_P(current), "data", sizeof("data")-1, 0, &rv);
            if (EG(exception)) {
                zend_string_release_ex(string_key, 0);
                return FAILURE;
            }
...
PHP_FUNCTION(swow_curl_copy_handle)
{
    php_curl    *ch;
    CURL        *cp;
    zval        *zid;
    ...
    ZEND_PARSE_PARAMETERS_START(1,1)
        Z_PARAM_OBJECT_OF_CLASS(zid, swow_curl_ce)
    ZEND_PARSE_PARAMETERS_END();
    ...
    dupch = swow_swow_init_curl_handle_into_zval(return_value);
    dupch->cp = cp;

    _swow_setup_easy_copy_handlers(dupch, ch);
...
# endif
...
#endif
        {
            ...
            if ((option == CURLOPT_PROTOCOLS_STR || option == CURLOPT_REDIR_PROTOCOLS_STR) &&
                (PG(open_basedir) && *PG(open_basedir))) {
                    zend_string *lower_str = zend_string_tolower(str);
                    if (ZSTR_LEN(lower_str) == sizeof("file") - 1 && memcmp(ZSTR_VAL(lower_str), "file", sizeof("file") - 1) == 0) {
                        zend_string_release_ex(lower_str, false);
                        php_error_docref(NULL, E_WARNING, "The FILE protocol cannot be activated when an open_basedir is set");
                        return FAILURE;
                    }
            }
        ...
        case CURLOPT_SHARE:
            {
                if (Z_TYPE_P(zvalue) == IS_OBJECT && Z_OBJCE_P(zvalue) == swow_curl_share_ce) {
...
PHP_FUNCTION(swow_curl_setopt_array)
{
    zval        *zid, *arr, *entry;
    php_curl    *ch;
    zend_ulong    option;
...
void _swow_curl_cleanup_handle(php_curl *ch)
{
...
PHP_FUNCTION(swow_curl_getinfo)
{
    zval        *zid;
    php_curl    *ch;
    zend_long    option;
...
static void curl_free_obj(zend_object *object)
{
    php_curl *ch = swow_curl_from_obj(object);
    ...
    _swow_curl_verify_handlers(ch, /* reporterror */ false);
...
PHP_FUNCTION(swow_curl_strerror)
{
    zend_long code;
    const char *str;
...
PHP_FUNCTION(swow_curl_unescape)
{
    char        *out = NULL;
    int          out_len;
    zval        *zid;
